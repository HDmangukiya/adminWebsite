...
public class BlogController {
    ...
    public Result save(@RequestParam("blogTitle") String blogTitle,
        ...
        if (!StringUtils.hasText(blogTitle)) {
            return ResultGenerator.genFailResult("Please enter a title for your article");
        }
        if (blogTitle.trim().length() > 150) {
            return ResultGenerator.genFailResult("The title is too long");
        }
        if (!StringUtils.hasText(blogTags)) {
            return ResultGenerator.genFailResult("Please enter the article tag");
        }
        if (blogTags.trim().length() > 150) {
            return ResultGenerator.genFailResult("The label is too long");
        }
        if (blogSubUrl.trim().length() > 150) {
            return ResultGenerator.genFailResult("The path is too long");
        }
        if (!StringUtils.hasText(blogContent)) {
            return ResultGenerator.genFailResult("Please enter the content of the article");
        }
        if (blogTags.trim().length() > 100000) {
            return ResultGenerator.genFailResult("The article is too long");
        }
        if (!StringUtils.hasText(blogCoverImage)) {
            return ResultGenerator.genFailResult("The cover image cannot be empty");
        }
        Blog blog = new Blog();
        ...
        if ("success".equals(saveBlogResult)) {
            return ResultGenerator.genSuccessResult("The addition was successful");
        } else {
    ...
    public Result update(@RequestParam("blogId") Long blogId,
        ...
        if ("success".equals(updateBlogResult)) {
            return ResultGenerator.genSuccessResult("The modification was successful");
        } else {
