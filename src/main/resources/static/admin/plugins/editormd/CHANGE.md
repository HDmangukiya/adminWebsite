...
- 新增配置项 `saveHTMLToTextarea` ，用于将解析后的 HTML 保存到 Textarea，以供Submit到后台程序；
...
- 修改了 `README.md` ，添加了 `Shields.io` Icon；
...
- 改进和增强工具栏自定义功能；
	- 新增配置项 `toolbarCustomIcons` ，用于增加自定义工具栏的功能，可以直接插入 HTML 标签，不使用默认的元素创建Icon；
...
- 修复 Font awesome Icon emoji 部分无法解析的 Bug，[#39](https://github.com/pandao/editor.md/issues/39)
...
- 修改 iPad 等移动终端的Browse器无法上传图片的问题 [#48](https://github.com/pandao/editor.md/issues/48)；
...
- 新增搜索工具栏按钮；
    ...
    - 原全屏预览 HTML 按钮的Icon改为 `fa-desktop`；
    - 改为默认开启搜索替换功能；
- 更换了Concerning Editor.md 的标语（ slogan ）；
- 标题按钮 `h` 改为大写的 `H`；
...
- 改进和增强自定义工具栏功能，支持Icon按钮右对齐 [#69](https://github.com/pandao/editor.md/issues/69)；
